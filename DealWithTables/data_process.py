    ...
    IpAddress, isp, Name from (
        select t1.*, ip.Name from (
            select a.日期, b.测试次数 as 总测试次数, a.丢包率平均值, cast(b.完全丢包次数 as float)/ b.测试次数 as 全丢包率, a.Ping的平均值的平均值, a.测试次数 as 有返回测试次数, b.完全丢包次数, a.Ping的标准差的平均值,
                a.IpAddress, a.isp, a.area
            from detail_report as a inner join complete_loss_detail as b
        on a.IpAddress = b.IpAddress and a.isp = b.isp and a.area = b.area and a.日期 = b.日期 and b.测试次数 > {times}) as t1 left outer join ip on t1.IpAddress = ip.IpAddress) as tt1 group by IpAddress, isp, 日期, Name
    '''
produce_report = '''
    ...
    IpAddress, Name from (
        select t1.*, ip.Name from (
            select a.日期, b.测试次数 as 总测试次数, a.丢包率平均值, cast(b.完全丢包次数 as float)/ b.测试次数 as 全丢包率, a.Ping的平均值的平均值, a.测试次数 as 有返回测试次数, b.完全丢包次数, a.Ping的标准差的平均值,
                a.IpAddress, a.isp, a.area
            from detail_report as a inner join complete_loss_detail as b
        on a.IpAddress = b.IpAddress and a.isp = b.isp and a.area = b.area and a.日期 = b.日期 and b.测试次数 > {times}) as t1 left outer join ip on t1.IpAddress = ip.IpAddress) as tt1 group by IpAddress, 日期, Name
    '''
test_sql = '''
    select * from (
    select t1.*, ip.Name from (
...
class MSSQL:
    ...
    def get_report(self, times=100):
        ...
        self.execute(produce_report_isp.format(times=times))
        self.execute(produce_report.format(times=times))
